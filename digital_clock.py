...
def date_time():
    ...
    hr = time.strftime("%I")#yeh hme hrs deta h
    mi = time.strftime("%M")
    sec = time.strftime("%S")
    am = time.strftime("%p")
    date = time.strftime("%d")#yeh date seta h
    month = time.strftime("%m")
    year = time.strftime("%y")
    day = time.strftime("%a")

    lab_hr.config(text=hr)
    lab_min.config(text=mi)##############################config hmare pass values ko change krta h
    ...
    lab_hr.after(200,date_time)#after==data ko change krta h

clock = Tk()
clock.title("    **** DIGITAL CLOCK ****")
clock.geometry("1000x500")
clock.config(bg="Sky Blue")
#*****TIME

lab_hr=Label(clock,text="00" ,font=("Time New Roman" ,60,"bold"),
             bg="Light Pink",fg="Purple")
...
lab_mnth=Label(clock,text="00" ,font=("Time New Roman" ,60,"bold"),
             bg="Light Pink",fg="Purple")
lab_mnth.place(x=340,y=270,height=110,width=100)
lab_mnth_txt=Label(clock,text="Month" ,font=("Time New Roman" ,20,"bold"),
             bg="Light Pink",fg="Purple")
lab_mnth_txt.place(x=340,y=410,height=40,width=100)
